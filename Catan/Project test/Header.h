...
public:
    ...
    void displayBoard(sf::RenderWindow& window) const {
        ...
        float posX = 477, posY = 98;
        float offsetX = 108, offsetY = 104;

        int currentTile = 0;
        for (int row = 0; row < 5; ++row) {
            for (int col = 0; col < (row % 2 == 0 ? 5 : 4); ++col) {
                sf::Sprite tile;
                switch (resources[currentTile][0]) {
                case 'S': tile.setTexture(sheep); break;
                case 'W': tile.setTexture(wood); break;
                case 'B': tile.setTexture(brick); break;
                case 'G': tile.setTexture(grain); break;
                case 'O': tile.setTexture(ore); break;
                }
...
void playerSteal(vector<Player*>& players, Player* a) {
    ...
    if (targetPlayer) {
        a->stealResource(*targetPlayer, targetPlayer->getResources(), resource, amount);
    }
    else {
